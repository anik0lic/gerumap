...
public class MessageGeneratorImplemetation implements MessageGenerator {
    ...
    private Message createMessage(EventType type) {
        switch (type) {
            ...
            case ENTER_INTEGER:
                return new Message("Please enter an integer", EventType.ERROR);
            case SELECT_ELEMENTS:
                return new Message("You must select elements", EventType.ERROR);
            case OPEN_MIND_MAP:
                return new Message("You must open mind map", EventType.ERROR);
            case INVALID_SIZE:
                return new Message("Please enter 1, 2 or 3 for stroke size", EventType.ERROR);
            case INVALID_NAME_SIZE:
                return new Message("Please enter a name that has less than 10 characters", EventType.ERROR);
            default:
                return null;
        }
    }
